...
import Tubnail from './Components/Tubnail'
import './App.css'
...
class App extends Component {
  ...
  isImgMatched = id => {
    ...
    if (imgUrl.id === id) {
      this.setState(prev => ({
        score: prev.score + 1,
        imgUrl: imagesList[randomNumber],
      }))
    } else {
      this.cleartime()
      this.setState({isTrue: true})
  ...
  matchgame = () => {
    const {imgUrl, activeTab} = this.state
    const filterImg = imagesList.filter(each => each.category === activeTab)
    /* const randomImge =
      imagesList[Math.ceil(Math.random() * imagesList.length - 1)] */

    return (
      <div className="match-container">
        <img src={imgUrl.imageUrl} alt="match" className="main-image" />
        <ul className="ul-tab">
          {tabsList.map(each => (
            <TabList
              key={each.tabId}
              ...
              inActiveTab={each.tabId === activeTab}
            />
          ))}
        </ul>
        <ul className="tumbnail-img">
          {filterImg.map(each => (
            <Tubnail
              key={each.id}
              tumbnail={each}
  ...
  gameOvercard = () => {
    ...
    return (
      <div className="game-over">
        ...
        <p className="your-score">Super da mapala</p>
        <button type="button" className="reset-game" onClick={this.restGame}>
          <img
  ...
  render() {
    const {score, seconds, isTrue} = this.state
    return (
      <div>
        <Navbar score={score} seconds={seconds} />
        <div className="container">
          {isTrue ? this.gameOvercard() : this.matchgame()}
        </div>
      </div>
    )
  }
